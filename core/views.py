...
def index(request):
    ...
    if request.method == 'POST':
        ...
        if Post.objects.filter(id=id, user=user).first():
            delete_post = Post.objects.get(id=id, user=user)
            delete_post.delete()
        else:
            return redirect('/')
    ...
    for user in user_following:
        ...
        following_user_all.append(user_list)

    new_suggestion_list = [x for x in list(all_users) if (x not in list(following_user_all))]
...
def like_post(request):
    ...
    print(like_filter)
    if like_filter == None:
        new_like = LikePost.objects.create(post_id=post_id,username=username).save()
